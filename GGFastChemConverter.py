...
# Reading in data
file = '/home/frank/Documents/MSc Research 1/Code/GGchem/Final Static Conc Files/Static_Conc_TP00.dat'
...
electrons_ind = header.index('e_minus')
...
for i in range(len(header)):
    ...
    if i > (supsat_indices[0] - 1) and i < condensates_indices[0]: # no action required for the superstaturation ratios
        pass
    if (i > supsat_indices[-1]) and (i < condensates_indices[-1] + 1): # converting the condensates 
        for j in range(len(col)):
            float_value = float(col[j])
            power_value = 10**float_value
            col[j] = power_value
...
for i in range(len(header)):
    ...
    if i > (electrons_ind - 1) and i < supsat_indices[0]: # gas species
        ...
        t.add_column(col)
    if i > (supsat_indices[0] - 1) and i < condensates_indices[0]: # supersaturation ratios
        pass
    if (i > supsat_indices[-1]) and (i < condensates_indices[-1] + 1): # condensates
        for j in range(len(col)):
            col[j] = col[j]/rowtotals[j]
        t.add_column(col)
    if i == (len(header) - 1):
